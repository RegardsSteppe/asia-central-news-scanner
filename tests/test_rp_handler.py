...
class HandlerTests(unittest.TestCase):
    ...
    def test_push_to_github_attaches_result_on_success(self, mock_push):
        mock_push.return_value = {
            ...
            "commit_sha": "abc123",
            "html_url": "https://github.com/owner/repo/blob/x",
        }

        result = handler(
            {
                "mode": "score",
                "articles": [dict(ACTIVIST_ARTICLE)],
                "push_to_github": {"path": "runpod_results/out.json"},
            }
        )

        self.assertEqual(result["scored"], 1)
        self.assertEqual(result["github_push"]["commit_sha"], "abc123")
    ...
    def test_push_to_github_failure_does_not_fail_the_job(self, mock_push):
        ...
        # Le scoring reste valide même si le push a échoué.
        self.assertEqual(result["scored"], 1)
    ...
    def test_push_to_github_true_uses_defaults(self, mock_push):
        ...
        mock_push.assert_called_once()
        self.assertEqual(mock_push.call_args.kwargs["branch"], "runpod-results")
