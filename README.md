...
* 🧠 **Persistent memory** — already-seen articles and each source's last-scan time are stored in `data/memory.json` (committed by the CI workflow), so a source scanned recently isn't re-fetched needlessly
...
├── .github/
│   └── workflows/                # news-scanner, fetch-bodies, find-sources, tests
│
├── matching.py                   # Shared matching primitives (no business logic)
├── keywords.py                   # Shared vocabulary, used by scoring AND categorisation
├── scoring.py                    # Deterministic relevance scoring, levels A-F
├── categorisation.py             # Descriptive categories — describes, never decides
├── regles_editoriales.py         # Editorial choices — decides, never detects
│
...
├── archive.py                    # Append-only article archive
├── fetch_all_bodies.py           # Bulk body fetcher, fills the archive
├── boilerplate.py                # Per-source footers, generated by tools/
├── text_utils.py                 # Text, date and URL normalization
├── http_utils.py                 # HTTP requests and in-memory caching
├── pays_monde.py                 # World-country vocabulary
│
├── html_template.py              # Dashboard HTML generation
├── github_push.py                # Publishing
├── synthesis.py                  # Optional LLM summary
├── juge_llm.py, rp_handler.py    # RunPod serverless (scoring + LLM judge)
├── verite_terrain.py             # Ground-truth comparison
├── find_candidate_sources.py     # Source discovery
│
├── data/
│   ├── archive.jsonl             # Append-only article archive (the measurement corpus)
│   ├── archive_state.json        # Derived archive state
│   └── memory.json               # Scanner memory, committed by CI on purpose
│
├── docs/
│   ├── HISTORY.md                # Architecture decisions and their rationale
│   ├── QUICKSTART.md
│   └── RUNPOD.md
│
├── tests/                        # unittest — what CI runs
├── tools/                        # One-off maintenance scripts
│
├── index.html                    # Generated dashboard
├── articles.csv                  # Generated article data, scores and matched keywords
├── Dockerfile, Dockerfile.juge   # RunPod images
├── requirements.txt              # Python dependencies
├── CLAUDE.md                     # Working instructions for AI agents
└── README.md
```
...
### Scanner memory (`data/memory.json`)

`data/memory.json` is committed by the GitHub Actions workflow after every run (`Persist scanner memory` step), so it survives across the ephemeral CI runners — unlike `http_cache.json`, it's tracked in git on purpose. It stores two things:
